...
func GetUser(w http.ResponseWriter, r *http.Request) {
	...
	if len(queryForm["Id"]) == 0 {
		models.SendRetJson(0, "失败", "缺少Id参数", w)
		return
	}
	...
	models.SendRetJson(0, "成功", user.ToString(), w)
}
...
func UpdateUser(w http.ResponseWriter, r *http.Request) {
	...
	if !ok || len(users) == 0 {
		models.SendRetJson(0, "手动滑稽", "缺少User参数", w)
		return
	}
	ids, ok := r.PostForm["Id"]
	if !ok || len(ids) == 0 {
		models.SendRetJson(0, "手动滑稽", "缺少Id参数", w)
		return
	}
	user := &models.User{}
	err := json.Unmarshal([]byte(users[0]), user)
	if err != nil {
		models.SendRetJson(0, "User格式错误", err.Error(), w)
		return
	}
